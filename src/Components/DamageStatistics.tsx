...
function PotencyDisplay(props: {
	...
	return (
		<span style={{ textDecoration: props.includeInStats ? "none" : "line-through" }}>
...
class DamageStatsSettings extends React.Component {
	...
	render() {
		let checkboxLabel = (
			<span>
				{localize({
					en: "exclude damage when untargetable",
					zh: "Boss上天期间威力按0计算",
				})}{" "}
				<Help
...
export class DamageStatistics extends React.Component {
	...
	render() {
		...
		let makeRow = function (props: {
			...
			return (
				<div key={props.key} style={rowStyle}>
		...
		let mainTable = (
			...
			>
				<div style={{ ...cell(100), ...{ textAlign: "center", marginBottom: 10 } }}>
					<b style={this.data.historical ? { color: colors.historical } : undefined}>
						{mainHeaderStr}
					</b>
				</div>
							...
							<span>
								...
								{this.data.mainTableSummary.totalPartyBuffPotency > 0 ? (
									<span style={{ color: colors.accent }}>
										{" "}
										+
										{this.data.mainTableSummary.totalPartyBuffPotency.toFixed(
											2,
										)}
										({localize({ en: "party", zh: "团辅" })})
									</span>
			...
			>
				...
				<div style={{ outline: "1px solid " + colors.bgMediumContrast }}>
									...
									content={localize({
										en: (
											<div>
												...
												<div className={"paragraph"}>
													The last row also includes gap at the beginning
						...
						<div style={{ display: "inline-block", width: "10%" }}>
							<span style={headerCellStyle}>
								...
								<Help
									...
									content={localize({
										en: (
											<div>Overridden DoT time from previous application</div>
										),
					...
					>
						<div style={cell(28)} />
						<div style={cell(10)}>
							{this.data.dotTableSummary.cumulativeGap.toFixed(3)}
						</div>
						<div style={cell(10)}>
		...
		return (
			<div>
				...
				<div>
					{mainTable}
					{(job === ShellJob.BLM || job === ShellJob.SAM || job === ShellJob.MCH) &&
						dotTable}
				</div>
			</div>
		);
