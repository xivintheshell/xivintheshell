...
export class TimelineMarkers extends React.Component {
	...
	render() {
		...
		let colorOption = function (markerColor: MarkerColor, displayName: ContentNode) {
			return (
				<option key={markerColor} value={markerColor}>
					{displayName}
				</option>
			);
		};
				...
				description={
					<>
						<span
							style={{
								color:
									isNaN(parsedTime) || parsedTime === 0 ? "" : MarkerColor.Purple,
							}}
						>
							{localize({
		...
		let loadTracksSection = (
			<>
				<LoadJsonFromFileOrUrl
					...
					defaultLoadUrl={""}
					label={localize({
						en: "Load multiple tracks combined: ",
						zh: "载入多轨文件：",
					})}
		...
		let addColumn = (
			<form>
				...
				>
					<option value={MarkerType.Info}>
						{localize({ en: "Info", zh: "备注信息" })}
					</option>
		...
		let presetsSection = (
			<div style={{}}>
				...
				<p>
					...
					<LoadCombinedTracksBtn
						displayName={localize({ en: "P1 by Yara", zh: "P1（英文，来自Yara）" })}
						url={PRESET_MARKERS_BASE + "fru_p1.txt"}
						offsetStr={this.state.offsetStr}
					/>
					<LoadCombinedTracksBtn
						displayName={localize({
							en: "P2 + intermission by Yara",
							zh: "P2（英文，来自Yara）",
						})}
					...
					/>
					<LoadCombinedTracksBtn
						displayName={localize({ en: "P3 by Yara", zh: "P3（英文，来自Yara）" })}
						url={PRESET_MARKERS_BASE + "fru_p3.txt"}
						offsetStr={this.state.offsetStr}
					/>
					<LoadCombinedTracksBtn
						displayName={localize({
							en: "full (zh) by 小盐 & czmm (12/2 ver)",
							zh: "完整（来自小盐+czmm, 12/2版）",
						})}
							...
							content: (
								<>
									...
									<p style={{ marginTop: 16 }}>
										<b>
											{localize({ en: "Load from file", zh: "从文件导入" })}
										</b>{" "}
										<Help
											topic={"load tracks"}
											content={localize({
