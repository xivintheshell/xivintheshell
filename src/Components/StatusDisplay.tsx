...
function ResourceBar(
	...
	return (
		<div hidden={props.hidden} style={{ marginBottom: 4, lineHeight: "1.5em" }}>
...
function ResourceCounter(props: {
	...
	return (
		<div className={props.className} style={{ marginBottom: 4, lineHeight: "1.5em" }}>
			<div style={{ display: "inline-block", height: "100%", width: 108 }}>{props.name}</div>
			<div style={{ width: 200, display: "inline-block" }}>
				<div style={{ display: "inline-block", marginLeft: 6 }}>{stacks}</div>
				<div style={{ marginLeft: 6, height: "100%", display: "inline-block" }}>
					{props.currentStacks + "/" + props.maxStacks}
				</div>
			</div>
		</div>
	);
...
function PaintGaugeCounter(props: {
	...
	for (let i = 0; i < props.maxStacks; i++) {
		...
		stacks.push(
			<ResourceStack
				key={i}
				color={isComet ? props.cometColor : props.holyColor}
				value={i < props.currentStacks}
			/>,
		);
	}
	return (
		<div style={{ marginBottom: 4, lineHeight: "1.5em" }}>
			<div style={{ display: "inline-block", height: "100%", width: 108 }}>{props.name}</div>
			<div style={{ width: 200, display: "inline-block" }}>
				<div style={{ display: "inline-block", marginLeft: 6 }}>{stacks}</div>
				<div style={{ marginLeft: 6, height: "100%", display: "inline-block" }}>
...
function SenCounter(props: {
	...
	return (
		<div style={{ marginBottom: 4, lineHeight: "1.5em" }}>
			<div style={{ display: "inline-block", height: "100%", width: 108 }}>
				{props.name} {help}
			</div>
			<div style={{ width: 200, display: "inline-block" }}>
				<div style={{ display: "inline-block", marginLeft: 6 }}>{stacks}</div>
...
export function ResourcesDisplay(props: {
	...
	const elements = props.data.resources.map((props, i) => {
		switch (props.kind) {
			...
			default:
				return (
					<ResourceText name={props.name} text={props.text} key={"resourceDisplay" + i} />
				);
		}
	});
	let style: CSSProperties = {
							...
							<div className="toolTip">
								{localize({
									en: (
										<>
											<div className="paragraph">
												<span style={{ color: "lightgray" }}>grey</span>{" "}
												border: not focused
											</div>
											<div className="paragraph">
												<b style={{ color: "mediumpurple" }}>purple</b>{" "}
												border: receiving input
											</div>
											<div className="paragraph">
												<b style={{ color: "mediumseagreen" }}>green</b>{" "}
												border: real-time
											</div>
											<div className="paragraph">
												<b style={{ color: "darkorange" }}>orange</b>{" "}
												border: viewing historical state, not receiving
												input
											</div>
										</>
									),
									ja: (
										<>
											<div className="paragraph">
												<span style={{ color: "lightgray" }}>グレー</span> :
												未選択
											</div>
											<div className="paragraph">
											...
											<div className="paragraph">
												<b style={{ color: "mediumseagreen" }}>緑</b> :
												リアルタイム
											</div>
											<div className="paragraph">
