...
export function ResourceOverrideDisplay(props: {
	...
	return (
		<div style={{ marginTop: 10, color: "mediumpurple" }}>
			{str}
			{deleteBtn}
		</div>
	);
		...
		en: (
			<div>
				<div className={"paragraph"}>
					The caster tax config is now replaced by 0.1s + FPS tax and is more precise. You
					can read more about FPS tax in the Github repository wiki's implementation notes
					section.
				</div>
				<div className={"paragraph"} style={{ color: warningColor }}>
					You are strongly encouraged to create a new record (in another timeline slot or
					from 'apply and reset') and migrate your fight plan. Support for loading legacy
					files might drop in the future.
				</div>
			</div>
		),
		zh: (
		...
		>
			...
			{props.job === ShellJob.BLM && (
				<div>
					...
					<Help topic={"thunderTickOffset"} content={thunderOffsetDesc} />:{" "}
					{thunderTickOffset}
				</div>
			)}
						...
						content={
							<div className="toolTip">
								...
								{localize({
									en: (
										<div className="paragraph">
											- click to use a skill. or if it's not ready, click
											again to wait then retry
										</div>
									),
									zh: (
										<div className="paragraph">
						...
						content={
							<div className="toolTip">
								...
								{localize({
									en: (
										<div className="paragraph">
											- <ButtonIndicator text={"space"} /> to advance game
											time to the earliest possible time for the next skill
										</div>
									),
									zh: (
										<div className="paragraph">
...
export class Config extends React.Component {
										...
										<div>
											...
											>
												<b>
													...
													Instructions/Troubleshoot first and use
													carefully at your own risk!
												</b>
											</div>
											<div className={"paragraph"}>
												Also, currently thunder dot buff created this way
												doesn't actually tick. It just shows the remaining
												buff timer.
											</div>
											<div className={"paragraph"}>
												I would recommend saving settings (stats, lines
												presets, timeline markers etc.) to files first, in
												case invalid game states really mess up the tool and
	...
	render() {
				...
				<table>
					<tbody>
						...
						<tr>
							<td>2.8</td>
							<td>
								{getTaxPreview(level, 2.8, this.state.spellSpeed, this.state.fps)}
							</td>
						</tr>
						<tr>
							<td>3.0</td>
							<td>
								{getTaxPreview(level, 3.0, this.state.spellSpeed, this.state.fps)}
							</td>
						</tr>
						<tr>
							<td>3.5</td>
							<td>
								{getTaxPreview(level, 3.5, this.state.spellSpeed, this.state.fps)}
							</td>
						</tr>
						<tr>
							<td>4.0</td>
							<td>
								{getTaxPreview(level, 4.0, this.state.spellSpeed, this.state.fps)}
							</td>
						</tr>
					</tbody>
				</table>
		...
		const etroLink = (
			<a href="https://etro.gg" target="_blank" rel="noreferrer">
				etro
			</a>
		);
		const xivgearLink = (
			<a href="https://xivgear.app" target="_blank" rel="noreferrer">
				xivgear
			</a>
		);
		const gearImportSection = (
						...
						<>
							<p>
								{localize({
									en: (
										<span>
											Enter and <ButtonIndicator text={"Load"} /> a link to a
											gearset from xivgear.app or etro.gg, edit the rest of
											config, then{" "}
			...
			<div style={{ marginBottom: 16 }}>
				...
				<div>
					...
					<span>
						...
						{this.state.b1TaxPreview}{" "}
						<Help topic={"b1TaxPreview"} content={b1TaxDesc} />)
					</span>
				</div>
				<Input
					componentColor={fpsAndCorrectionColor}
