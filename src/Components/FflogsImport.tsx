 ...
 */
async function initiateFflogsAuth() {
...
function parseLogURL(urlString: string): LogQueryParams {
	...
	return {
		apiBaseUrl: `https://${url.hostname}/api/v2/client/`,
		reportCode,
		fightID,
		playerID,
	};
...
export function FflogsImportFlow() {
	...
	const [flowState, setFlowState] = useState<LogImportFlowState>(
		...
		window.localStorage.getItem("fflogsAuthToken") === null
	...
	useEffect(() => {
		...
		// On component load, check if a code was specified in the parameter from PKCE authorization.
			...
			.then((response) => {
				if (response.ok) {
					response.json().then((tokens) => {
						...
						window.localStorage.setItem("fflogsAuthToken", tokens["access_token"]);
						console.log("saved auth tokens to localStorage");
						// TODO add intermediate state to show user success?
