...
type PotencyModifier = {
	factor: number,
	source: "AF1" | "AF2" | "AF3" | "UI1" | "UI2" | "UI3" | "eno" | "pot"
}
...
export function getPotencyModifiersFromResourceState(resources: ResourceState, aspect: Aspect) : PotencyModifier[] {
	...
	if (resources.get(ResourceType.Tincture).available(1)) {
		mods.push({source: "pot", factor: 1});
	}

	// eno
	if (resources.get(ResourceType.Enochian).available(1)) {
		if (!Debug.noEnochian) mods.push({source: "eno", factor: 1.21});
	}

	// ui1
	let ui = resources.get(ResourceType.UmbralIce);
	if (ui.availableAmount() === 1) {
		if (aspect === Aspect.Fire) mods.push({source: "UI1", factor: 0.9});
	}
	// ui2
	else if (ui.availableAmount() === 2) {
		if (aspect === Aspect.Fire) mods.push({source: "UI2", factor: 0.8});
	}
	// ui3
	else if (ui.availableAmount() === 3) {
		if (aspect === Aspect.Fire) mods.push({source: "UI3", factor: 0.7});
	}
	...
	if (af.availableAmount() === 1) {
		if (aspect === Aspect.Ice) {
			mods.push({source: "AF1", factor: 0.9});
		}  else if (aspect === Aspect.Fire) {
			mods.push({source: "AF1", factor: 1.4});
		}
	}
	// af2
	else if (af.availableAmount() === 2) {
		if (aspect === Aspect.Ice) {
			mods.push({source: "AF2", factor: 0.8});
		}  else if (aspect === Aspect.Fire) {
			mods.push({source: "AF2", factor: 1.6});
		}
	}
	// af3
	else if (af.availableAmount() === 3) {
		if (aspect === Aspect.Ice) {
			mods.push({source: "AF3", factor: 0.7});
		}  else if (aspect === Aspect.Fire) {
			mods.push({source: "AF3", factor: 1.8});
		}
	}
...
export class Potency {
	...
	getAmount(props: {tincturePotencyMultiplier: number}) {
		...
		this.modifiers.forEach(m=>{
			if (m.source==="pot") potency *= props.tincturePotencyMultiplier;
