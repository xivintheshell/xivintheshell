...
export class DNCState extends GameState {
	...
	applyStandardFinish(bonusLevel: number) {
		...
		const duration = (getResourceInfo("DNC", ResourceType.StandardFinish) as ResourceInfo)
			.maxTimeout;
...
technicalFinishes.forEach((params) => {
	makeGCD_DNC(params.skill, 70, {
		...
		onConfirm: (state) => {
			...
			const duration = (getResourceInfo("DNC", ResourceType.TechnicalFinish) as ResourceInfo)
				.maxTimeout;
...
makeAbility_DNC(SkillName.Improvisation, 80, ResourceType.cd_Improvisation, {
	...
	onConfirm: (state) => {
		...
		state.resources.addResourceEvent({
			...
			delay: (getResourceInfo("DNC", ResourceType.Improvisation) as ResourceInfo).maxTimeout,
			fnOnRsc: (rsc) => {
				rsc.consume(1);
