...
import {BuffProps, ResourceBarProps, ResourceCounterProps, ResourceDisplayProps} from "../../Components/StatusDisplay";
...
export class BLMState extends GameState {
	...
	override getEnemyBuffViewProps(): BuffProps[] {
		const DoTCountdown = this.resources.timeTillReady(ResourceType.ThunderDoT);
		const addleCountdown = this.resources.timeTillReady(ResourceType.Addle);
		return [
			{
	...
	override getResourceViewProps(): ResourceDisplayProps[] {
		...
		let eno = this.resources.get(ResourceType.Enochian);
		...
		const astralFire = this.getFireStacks();
		const umbralIce = this.getIceStacks();
		...
		if (Traits.hasUnlocked(TraitName.EnhancedAstralFire, this.config.level)) {
			infos.push({
				kind: "counter",
				name: localize({
					en: "astral soul",
