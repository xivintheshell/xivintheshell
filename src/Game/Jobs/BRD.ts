...
export class BRDState extends GameState {
    ...
    songTick(song: ResourceType) {
        ...
        if (this.resources.timeTillReady(song) > 3) {
            this.addEvent(new Event(`${song} tick`, 3, () => { this.songTick(song) }))
        }
    }
    ...
    gainRepertoireEffect(song?: ResourceType) {
        ...
        if (!song) {
            if (this.hasResourceAvailable(ResourceType.WanderersMinuet)) { song = ResourceType.WanderersMinuet}
            else if (this.hasResourceAvailable(ResourceType.MagesBallad)) { song = ResourceType.MagesBallad }
            else if (this.hasResourceAvailable(ResourceType.ArmysPaeon)) { song = ResourceType.ArmysPaeon }
        }
    ...
    beginSong(newSong: ResourceType)
    {
        ...
        if (Traits.hasUnlocked(TraitName.MinstrelsCoda, this.config.level)) {
            const coda = (newSong === ResourceType.WanderersMinuet) ? ResourceType.WanderersCoda :
                (newSong === ResourceType.MagesBallad) ? ResourceType.MagesCoda : ResourceType.ArmysCoda
        ...
        this.addEvent(new Event(`${newSong} tick`, 3, () => { this.songTick(newSong) }))
    }

    tryExpireSong(song: ResourceType) {
        if (!this.resourceIsSong(song)) { return }
        if (!this.hasResourceAvailable(song)) { return }
...
const makeAbility_BRD = (name: SkillName, unlockLevel: number, cdName: ResourceType, params: {
    potency?: number | Array<[TraitName, number]> | ResourceCalculationFn<BRDState>
    replaceIf?: ConditionalSkillReplace<BRDState>[],
    highlightIf?: StatePredicate<BRDState>,
    startOnHotbar?: boolean,
...
const makeResourceAbility_BRD = (name: SkillName, unlockLevel: number, cdName: ResourceType, params: {
...
makeWeaponskill_BRD(SkillName.RefulgentArrow, 70, {
    ...
    validateAttempt: (state) => state.hasResourceAvailable(ResourceType.HawksEye) || state.hasResourceAvailable(ResourceType.Barrage),
    highlightIf: (state) => state.hasResourceAvailable(ResourceType.HawksEye) || state.hasResourceAvailable(ResourceType.Barrage), 
})
...
makeWeaponskill_BRD(SkillName.ApexArrow, 80, {
    ...
    onConfirm: (state) => {
        if (Traits.hasUnlocked(TraitName.EnhancedApexArrow, state.config.level) && 
...
makeAbility_BRD(SkillName.PitchPerfect, 52, ResourceType.cd_PitchPerfect, {
    ...
    potency: (state) => {
        ...
        return (pitchPerfectStacks === 3) ? 360 :
            (pitchPerfectStacks === 2) ? 220 : 100
    },
    onConfirm: (state) => state.tryConsumeResource(ResourceType.PitchPerfect, true),
    validateAttempt: (state) => state.hasResourceAvailable(ResourceType.WanderersMinuet) && state.hasResourceAvailable(ResourceType.PitchPerfect),
    highlightIf: (state) => state.hasResourceAvailable(ResourceType.WanderersMinuet) && state.hasResourceAvailable(ResourceType.PitchPerfect)
})
...
makeWeaponskill_BRD(SkillName.RadiantEncore, 100, {
    ...
    potency: (state) => {
        ...
        return (radiantCoda === 3 ) ? 900 :
            (radiantCoda === 2) ? 600 : 500
    },
...
makeWeaponskill_BRD(SkillName.QuickNock, 18, {
    ...
    onApplication: (state) => state.maybeGainProc(ResourceType.HawksEye, 0.20),
    autoUpgrade: {
