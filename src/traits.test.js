...
expect.extend({
	toBeGreaterExt([name, value], expected) {
		return {
			message: () =>
				`${name} = ${value}. Must be greater than ${expected} (values 0-${expected} are reserved)`,
			pass: value > expected,
		};
	},
	toBeUnique([name, value], map) {
		return {
			message: () =>
				`${name} conflicts with ${map.get(value)}. Both have a value of ${value}.`,
			pass: !map.has(value),
		};
	},
	toBeLockedAtLevel0([name, value]) {
		return {
			message: () =>
				`${name} is missing from Traits lookup. Make sure it is included in the TraitList`,
